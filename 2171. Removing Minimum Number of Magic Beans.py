"""
LeetCode #2171 - Removing Minimum Number of Magic Beans
拿出最少数目的魔法豆
https://leetcode.cn/problems/removing-minimum-number-of-magic-beans/

给定一个 正整数 数组 `beans` ，其中每个整数表示一个袋子里装的魔法豆的数目。
请你从每个袋子中 拿出 一些豆子（也可以 不拿出），使得剩下的 非空 袋子中（即 至少还有一颗 魔法豆的袋子）魔法豆的数目 相等。一旦把魔法豆从袋子中取出，你不能再将它放到任何袋子中。
请返回你需要拿出魔法豆的 最少数目。

示例 1：
输入：beans = [4,1,6,5] 输出：4 解释： - 我们从有 1 个魔法豆的袋子中拿出 1 颗魔法豆。   剩下袋子中魔法豆的数目为：[4,0,6,5] - 然后我们从有 6 个魔法豆的袋子中拿出 2 个魔法豆。   剩下袋子中魔法豆的数目为：[4,0,4,5] - 然后我们从有 5 个魔法豆的袋子中拿出 1 个魔法豆。   剩下袋子中魔法豆的数目为：[4,0,4,4] 总共拿出了 1 + 2 + 1 = 4 个魔法豆，剩下非空袋子中魔法豆的数目相等。 没有比取出 4 个魔法豆更少的方案。
示例 2：
输入：beans = [2,10,3,2] 输出：7 解释： - 我们从有 2 个魔法豆的其中一个袋子中拿出 2 个魔法豆。   剩下袋子中魔法豆的数目为：[0,10,3,2] - 然后我们从另一个有 2 个魔法豆的袋子中拿出 2 个魔法豆。   剩下袋子中魔法豆的数目为：[0,10,3,0] - 然后我们从有 3 个魔法豆的袋子中拿出 3 个魔法豆。   剩下袋子中魔法豆的数目为：[0,10,0,0] 总共拿出了 2 + 2 + 3 = 7 个魔法豆，剩下非空袋子中魔法豆的数目相等。 没有比取出 7 个魔法豆更少的方案。

提示：
`1 <= beans.length <= 10^5`
`1 <= beans[i] <= 10^5`
"""

from typing import List, Optional


class Solution:
    def minimumRemoval(self, beans: List[int]) -> int:
        beans.sort()
        n = len(beans)
        total = sum(beans)
        min_removal = float('inf')

        for i in range(n):
            # Keep beans[i] as the target value
            # Remove all bags from 0..i-1 (they become 0)
            # Reduce bags i..n-1 to beans[i]
            removal = total - beans[i] * (n - i)
            min_removal = min(min_removal, removal)

        return min_removal



# ═══════════════════════════════════════════════════════════════════════════════
# Solution & Explanation
# ═══════════════════════════════════════════════════════════════════════════════
#
# Difficulty: Medium
# Tags: Greedy, Array, Enumeration, Prefix Sum, Sorting
#
# 解题思路:
# 先对 beans 排序。最终所有非空袋子保留的魔法豆数量一定是 beans 中的某个值（设为
# beans[i]）。枚举每个 beans[i] 作为目标值：由于数组已排序，所有索引 < i 的袋子值更
# 小，把它们全部清空最划算（拿出 beans[j] 个）；所有索引 >= i 的袋子值更大或相等，需要
# 减少到 beans[i]（拿出 beans[j] - beans[i] 个）。总拿出量为 total - beans[i] * (n - i)。
# 遍历所有 i 取最小值即可。
#
# 时间复杂度: O(n log n)
# 空间复杂度: O(1)（不计排序所需的额外空间）
#
# 关键点:
# - 排序后枚举，最终目标值必定是 beans 中的某个值
# - 公式推导：移除量 = 总和 - 目标值 * 剩余袋子数量
# - 排序保证前面的袋子值更小故全部清空，后面的袋子减少到目标值
# - 贪心思想：保留的值越大，剩余袋子越多，但清空前面小袋子的代价也越高
