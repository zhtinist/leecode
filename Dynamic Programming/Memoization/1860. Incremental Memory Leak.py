...
请你返回一个数组，包含 `[crashTime, memory1_crash, memory2_crash]` ，其中 `crashTime`是程序意外退出的时间（单位为秒）， `memory1_crash` 和 `memory2_crash` 分别是两个内存条最后剩余内存的位数。
...
#
# 解题思路:
# 模拟内存分配过程。每秒选择剩余内存较多的内存条分配 i 位内存。
# 使用 while 循环，每次迭代比较两个内存条的剩余空间，选择较大的
# 进行分配（相等时选第一个）。当两个内存条都无法满足当前需求时退出。
#
# 时间复杂度: O(sqrt(memory1 + memory2)) — 循环次数约为 sqrt(2 * (memory1 + memory2))
# 空间复杂度: O(1) — 只使用常数变量
#
# 关键点:
# - 当两者相等时优先分配给第一个内存条
# - 循环退出条件是两者剩余内存都不足 i 位
# - 每次分配后 i 递增 1
