...
The demons captured the princess and imprisoned her in the bottom-right corner
of a dungeon. The dungeon consists of m x n rooms laid out in a 2D grid. Our
brave knight was initially positioned in the top-left room and must fight his
way through dungeon to rescue the princess.

The knight has an initial health point represented by a positive integer. If at
any time his health point drops to 0 or below, he dies immediately.
    ...
    def calculateMinimumHP(self, dungeon: List[List[int]]) -> int:
        ...
        for i in range(m - 1, -1, -1):
            for j in range(n - 1, -1, -1):
                ...
                need = max(1, min_next - dungeon[i][j])
                dp[j] = need
