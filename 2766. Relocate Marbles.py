...
#
# 解题思路:
# 题目只关心哪些位置有石块，不关心每个位置有多少个。使用集合存储有石块的位置。
# 每次操作：从集合中移除 moveFrom[i]，添加 moveTo[i]。
# 最终返回排序后的集合即可。
#
# 时间复杂度: O((n+m) log(n+m)) 其中 n=len(nums), m=len(moveFrom)，主要是排序
# 空间复杂度: O(n) 存储集合
#
# 关键点:
# - 不需要追踪每个位置有多少石块，只需要知道哪些位置有石块
# - 使用 set 去重，每次操作从集合中移除和添加
# - 操作顺序重要：先移除再添加（如果 moveFrom == moveTo 也安全）
