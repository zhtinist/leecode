...
给你一个有 `n` 个服务器的计算机网络，服务器编号为 `0` 到 `n - 1` 。同时给你一个二维整数数组 `edges` ，其中 `edges[i] = [u_i, v_i]` 表示服务器 `u_i` 和 `v_i`_ 之间有一条信息线路，在 一秒 内它们之间可以传输 任意 数目的信息。再给你一个长度为 `n` 且下标从 0 开始的整数数组 `patience` 。
...
class Solution:
    def networkBecomesIdle(self, edges: List[List[int]], patience: List[int]) -> int:
        ...
        # BFS to find shortest distance from master server (0) to each data server
        dist = [0] * n
        queue = deque([0])
        visited = [False] * n
        visited[0] = True
        ...
        for i in range(1, n):
            round_trip = 2 * dist[i]
            p = patience[i]
            # Number of resends before reply arrives
            # The last resend happens at time: floor((round_trip - 1) / p) * p
            last_resend = ((round_trip - 1) // p) * p
            idle_time = last_resend + round_trip + 1
...
#
# 解题思路:
# 使用BFS计算每个数据服务器到主服务器（0号）的最短距离dist[i]。
# 往返时间 = 2 * dist[i]。在收到回复前，服务器i每patience[i]秒重发一次。
# 最后一次重发的时间 = ((round_trip - 1) // patience[i]) * patience[i]。
# 该服务器的空闲时间 = 最后重发时间 + 往返时间 + 1。
# 全局空闲时间是所有服务器中最大的空闲时间。
#
# 时间复杂度: O(n + m) 其中m是边数
# 空间复杂度: O(n + m)
#
