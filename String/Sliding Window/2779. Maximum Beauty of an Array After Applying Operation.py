...
#
# 解题思路:
# 每个元素 nums[i] 可以变成 [nums[i]-k, nums[i]+k] 范围内的任意值。
# 要最大化相等元素的子序列长度，需要找到一个值能被尽可能多的区间覆盖。
# 问题等价于：排序后，找最长子数组使得 max - min <= 2k（因为这两个元素可以变成相同的值）。
# 滑动窗口：右指针扩展，当窗口跨度 > 2k 时收缩左指针。
#
# 时间复杂度: O(n log n)
# 空间复杂度: O(1) 或 O(n) 取决于排序
#
# 关键点:
# - 将问题转化为排序后滑动窗口：能变成相同值的元素必须在 2k 范围内
# - 子序列不要求连续，但排序后滑动窗口等价于找最大交集
# - 两个元素能变成相同值的条件是 |nums[i] - nums[j]| <= 2k
