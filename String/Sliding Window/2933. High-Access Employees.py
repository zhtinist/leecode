...
示例 2：
输入：access_times = [["d","0002"],["c","0808"],["c","0829"],["e","0215"],["d","1508"],["d","1444"],["d","1410"],["c","0809"]] 输出：["c","d"] 解释："c" 在时间段 [08:08, 09:07] 内有三条访问记录，时间分别为 08:08 、08:09 和 08:29 。 "d" 在时间段 [14:10, 15:09] 内有三条访问记录，时间分别为 14:10 、14:44 和 15:08 。 然而，"e" 只有一条访问记录，因此不能包含在答案中，最终答案是 ["c","d"] 。
示例 3：
输入：access_times = [["cd","1025"],["ab","1025"],["cd","1046"],["cd","1055"],["ab","1124"],["ab","1120"]] 输出：["ab","cd"] 解释："ab"在时间段 [10:25, 11:24] 内有三条访问记录，时间分别为 10:25 、11:20 和 11:24 。 "cd" 在时间段 [10:25, 11:24] 内有三条访问记录，时间分别为 10:25 、10:46 和 10:55 。 因此，答案是 ["ab","cd"] 。
...
class Solution:
    def findHighAccessEmployees(self, access_times: List[List[str]]) -> List[str]:
        ...
        for name, time_str in access_times:
            minutes = int(time_str[:2]) * 60 + int(time_str[2:])
...
#
# 解题思路:
# 将每个员工的访问时间转换为分钟数，按员工分组并排序。对于每个员工，检查排序后的时间列表中是否存在连续3个访问时间的间隔 < 60 分钟。
# 若是，则该员工为高访问员工。使用滑动窗口检查 t[i+2] - t[i] < 60。
#
# 时间复杂度: O(m log m) 其中 m 为总访问次数
# 空间复杂度: O(m)
#
# 关键点:
# - 时间转换为分钟数便于比较
# - 同一小时内 = 最早和最晚时间差 < 60 分钟
# - 排序后滑动窗口检查连续3个即可
