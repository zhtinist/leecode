...
class Solution:
    def getSubarrayBeauty(self, nums: List[int], k: int, x: int) -> List[int]:
        # nums[i] in range [-50, 50], small range -> use counting sort approach
        OFFSET = 50
        size = 101  # -50 to 50
        freq = [0] * size
        n = len(nums)
...
#
# 解题思路:
# 由于nums[i]范围只有[-50, 50]，使用计数排序思路维护滑动窗口的频率数组。
# 滑动窗口大小为k，每次添加新元素并移除窗口外元素。对于每个窗口，
# 从小到大累加频率直到达到x，如果第x小的数是负数则返回该数，否则返回0。
#
# 时间复杂度: O(n * 101) = O(101n) ≈ O(n)
# 空间复杂度: O(101) = O(1)
#
# 关键点:
# - 值域很小(-50到50)，使用频率数组替代排序
# - 偏移量OFFSET=50将负数索引转为非负
# - 只有负数才作为美丽值返回，正数或零返回0
