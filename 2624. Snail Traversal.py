...
class Solution:
    ...
    def snailTraversal(self, nums: List[int], rowsCount: int, colsCount: int) -> List[List[int]]:
        ...
        for col in range(colsCount):
            if col % 2 == 0:
                ...
                for row in range(rowsCount):
                    result[row][col] = nums[col * rowsCount + row]
            else:
                # bottom to top
                for row in range(rowsCount - 1, -1, -1):
                    result[row][col] = nums[col * rowsCount + (rowsCount - 1 - row)]
...
#
# 解题思路:
# 首先检查rowsCount * colsCount是否等于数组长度，不等则返回空数组。然后按列遍历：
# 偶数列从上到下填充，奇数列从下到上填充。根据列号计算元素在一维数组中的索引。
# 这是"蜗牛排序"（或称蛇形填充）的逆过程——将一维数组按蛇形填入二维矩阵。
#
# 时间复杂度: O(rowsCount * colsCount)
# 空间复杂度: O(1) 不计输出数组
#
# 关键点:
# - 先验证rowsCount * colsCount == len(nums)
# - 偶数列从上到下，奇数列从下到上
# - 索引计算: col * rowsCount + row（从上到下时）
