...
class Solution:
    def wateringPlants(self, plants: List[int], capacity: int) -> int:
        ...
        for i, need in enumerate(plants):
            if water < need:
                # Go back to river, refill, come back
                steps += 2 * i  # go back and forth to river
                water = capacity
            # Water this plant
            water -= need
            steps += 1  # step to this plant
        return steps
...
#
# 解题思路:
# 模拟浇水过程。从河边(-1)开始，每次向前走一步到植物i处。
# 如果当前水量不足以浇灌该植物，则需要返回河边(2*i步来回)补充水，再重新走到该植物。
# 浇水后水量减少，步数加1。
#
# 时间复杂度: O(n)
# 空间复杂度: O(1)
