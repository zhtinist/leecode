...
解释：输入二进制字符串 00000010100101000001111010011100 表示无符号整数 43261596，因此返回 964176192，其二进制表示为 00111001011110000010100101000000。

示例 2：

输入：11111111111111111111111111111101
输出：10111111111111111111111111111111
解释：输入二进制字符串 11111111111111111111111111111101 表示无符号整数 4294967293，因此返回 3221225471，其二进制表示为 10101111110010110010011101101001。

注意：

请注意，在某些语言（如 Java）中没有无符号整数类型。在这种情况下，输入和输出都将作为有符号整数类型给出，不应影响你的实现，因为整数的内部二进制表示在有无符号时是一样的。

在 Java 中，编译器使用 2 的补码表示法表示有符号整数。因此，在上面的示例 2 中，输入表示有符号整数 `-3`，输出表示有符号整数 `-1073741825`。
