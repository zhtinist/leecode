...
class Solution:
    def decodeCiphertext(self, encodedText: str, rows: int) -> str:
        ...
        cols = n // rows
        # Build matrix row by row from encodedText
        mat = []
        for i in range(rows):
            mat.append(list(encodedText[i * cols:(i + 1) * cols]))

        # Read diagonally: starting from (0, c) for c = 0 to cols-1
        result = []
        for c in range(cols):
            r = 0
            while r < rows and c + r < cols:
                result.append(mat[r][c + r])
                r += 1
...
#
# 解题思路:
# 将encodedText按rows行填充到矩阵中（每行cols = len(encodedText) // rows个字符）。
# 解码过程是按对角线从左到右读取：从第一行的每一列c开始，
# 沿对角线向右下方向(r, c+r)读取字符，直到超出矩阵边界。
# 最后去除尾随空格得到originalText。
#
# 时间复杂度: O(n)
# 空间复杂度: O(n)
#
