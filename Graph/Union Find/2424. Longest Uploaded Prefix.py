"""
LeetCode #2424 - Longest Uploaded Prefix
最长上传前缀
https://leetcode.cn/problems/longest-uploaded-prefix/

给你一个 `n` 个视频的上传序列，每个视频编号为 `1` 到 `n` 之间的 不同 数字，你需要依次将这些视频上传到服务器。请你实现一个数据结构，在上传的过程中计算 最长上传前缀 。
如果 闭区间 `1` 到 `i` 之间的视频全部都已经被上传到服务器，那么我们称 `i` 是上传前缀。最长上传前缀指的是符合定义的 `i` 中的 最大值 。

请你实现 `LUPrefix` 类：
`LUPrefix(int n)` 初始化一个 `n` 个视频的流对象。
`void upload(int video)` 上传 `video` 到服务器。
`int longest()` 返回上述定义的 最长上传前缀 的长度。

示例 1：
输入： ["LUPrefix", "upload", "longest", "upload", "longest", "upload", "longest"] [[4], [3], [], [1], [], [2], []] 输出： [null, null, 0, null, 1, null, 3]  解释： LUPrefix server = new LUPrefix(4);   // 初始化 4个视频的上传流 server.upload(3);                    // 上传视频 3 。 server.longest();                    // 由于视频 1 还没有被上传，最长上传前缀是 0 。 server.upload(1);                    // 上传视频 1 。 server.longest();                    // 前缀 [1] 是最长上传前缀，所以我们返回 1 。 server.upload(2);                    // 上传视频 2 。 server.longest();                    // 前缀 [1,2,3] 是最长上传前缀，所以我们返回 3 。

提示：
`1 <= n <= 10^5`
`1 <= video <= 10^5`
`video` 中所有值 互不相同 。
`upload` 和 `longest` 总调用 次数至多不超过 `2 * 10^5` 次。
至少会调用 `longest` 一次。
"""

from typing import List, Optional


class LUPrefix:
    def __init__(self, n: int):
        self.uploaded = [False] * (n + 2)  # 1-indexed, extra slot for bound check
        self._longest = 0

    def upload(self, video: int) -> None:
        self.uploaded[video] = True
        # Greedily extend the longest prefix as far as possible
        while self.uploaded[self._longest + 1]:
            self._longest += 1

    def longest(self) -> int:
        return self._longest



# ═══════════════════════════════════════════════════════════════════════════════
# Solution & Explanation
# ═══════════════════════════════════════════════════════════════════════════════
#
# Difficulty: Medium
# Tags: Union Find, Design, Binary Indexed Tree, Segment Tree, Hash Table, Binary Search, Ordered Set, Heap (Priority Queue)
#
# 解题思路:
# 使用一个布尔数组 uploaded 标记已上传的视频（1-indexed），
# 并用一个指针 _longest 追踪当前的最长上传前缀。
# 每次 upload(video) 时标记为已上传，然后贪心地将 _longest 指针
# 向后推进：只要 _longest+1 已上传，就递增 _longest。
# longest() 直接返回 _longest 即可。
#
# 时间复杂度: O(1) 均摊 — 每个视频最多被推进一次 _longest 指针
# 空间复杂度: O(n) — 布尔数组存储 n 个视频的上传状态
#
# 关键点:
# - _longest 指针单调递增，每个位置最多被检查一次
# - 数组大小设为 n+2 避免越界（检查 _longest+1 时）
# - upload 和 longest 均为 O(1) 操作
