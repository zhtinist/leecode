...
class Solution:
    def assignTasks(self, servers: List[int], tasks: List[int]) -> List[int]:
        ...
        for j in range(m):
            ...
            ans[j] = idx
            heappush(busy, (time + tasks[j], w, idx))
...
# 解题思路:
# 使用两个最小堆模拟任务调度：
# 1. available 堆：存储空闲服务器，按 (权重, 下标) 排序。
# 2. busy 堆：存储忙碌服务器，按 (释放时间, 权重, 下标) 排序。
# 3. 按时间顺序处理任务 j（到达时间 = j）：
#    - 将 busy 中释放时间 <= 当前时间的服务器移回 available。
#    - 如果 available 为空，快进时间到下一个服务器释放的时间。
#    - 从 available 中取出最优服务器分配任务。
#    - 将服务器加入 busy，释放时间 = 当前时间 + tasks[j]。
#
# 时间复杂度: O((n + m) log n) — 每个服务器最多入堆出堆 m 次
# 空间复杂度: O(n) — 两个堆的大小
#
# 关键点:
# - 任务按顺序到达，但可以等待空闲服务器
# - available 堆按 (权重, 下标) 排序
# - busy 堆按 (释放时间, 权重, 下标) 排序
# - 需要处理时间跳跃（当没有可用服务器时）
