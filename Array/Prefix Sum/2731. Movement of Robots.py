...
#
# 解题思路:
# 关键洞察：机器人相撞后反向等同于彼此穿过（因为机器人不可区分）。
# 因此每个机器人最终位置 = 初始位置 + d（向右）或 -d（向左）。
# 计算所有位置并排序，然后使用前缀和快速计算两两距离之和：
# sum_{i} (pos[i] * i - prefix[i-1])，其中 prefix[i-1] 是前 i 个元素的和。
#
# 时间复杂度: O(n log n) 主要在于排序
# 空间复杂度: O(n) 存储位置数组
#
# 关键点:
# - 碰撞等效于穿过：因为机器人不可区分，碰撞后方向反转相当于两者交换身份继续前进
# - 两两距离和 = sum(pos[i] * i - prefix[i-1]) 用 O(n) 计算
# - 注意取模运算和负数的处理
