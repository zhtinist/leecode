...
表：`UserActivity`
+------------------+---------+ | Column Name      | Type    |  +------------------+---------+ | user_id          | int     | | activity_date    | date    | | activity_type    | varchar | | activity_duration| int     | +------------------+---------+ (user_id, activity_date, activity_type) 是这张表的唯一主键。 activity_type 是('free_trial', 'paid', 'cancelled')中的一个。 activity_duration 是用户当天在平台上花费的分钟数。 每一行表示一个用户在特定日期的活动。
...
def analyze_subscription_conversion(user_activity: pd.DataFrame) -> pd.DataFrame:
    # Filter users who have both free_trial and paid
    user_types = user_activity.groupby("user_id")["activity_type"].apply(set)
    converted_users = user_types[user_types.apply(lambda x: "free_trial" in x and "paid" in x)].index

    result = []
    for uid in converted_users:
        user_data = user_activity[user_activity["user_id"] == uid]
        trial = user_data[user_data["activity_type"] == "free_trial"]
        paid = user_data[user_data["activity_type"] == "paid"]
        trial_avg = round(trial["activity_duration"].mean(), 2)
        paid_avg = round(paid["activity_duration"].mean(), 2)
        result.append({
            "user_id": uid,
...
#
# 解题思路:
# 1. 按 user_id 分组，找出同时有 free_trial 和 paid 活动的用户（即转化为付费的用户）
# 2. 对每个转化用户：
#    - 筛选 free_trial 记录，计算平均每日活动时长
#    - 筛选 paid 记录，计算平均每日活动时长
#    - 四舍五入到小数点后 2 位
# 3. 结果按 user_id 升序排序
#
# 时间复杂度: O(n) — groupby 和遍历均为线性
# 空间复杂度: O(n)
#
# 关键点:
# - 只有同时有 free_trial 和 paid 的用户才包含在输出中
# - cancelled 用户不算转化，不包含在结果中
# - rounding 到 2 位小数
