...
示例 2：
输入： t = 20 calls = [   {"t": 50, inputs: [1]},   {"t": 100, inputs: [2]} ] 输出：[{"t": 70, inputs: [1]}, {"t": 120, inputs: [2]}] 解释： 第一次调用延迟到 70ms。输入为 (1)。 第二次调用延迟到 120ms。输入为 (2)。
示例 3：
输入： t = 150 calls = [   {"t": 50, inputs: [1, 2]},   {"t": 300, inputs: [3, 4]},   {"t": 300, inputs: [5, 6]} ] 输出：[{"t": 200, inputs: [1,2]}, {"t": 450, inputs: [5, 6]}] 解释： 第一次调用延迟了 150ms，运行时间为 200ms。输入为 (1, 2)。 第二次调用被第三次调用取消 第三次调用延迟了 150ms，运行时间为 450ms。输入为 (5, 6)。
...
class Solution:
    ...
    def debounce(self, fn, t: int):
        ...
        def debounced(*args):
            ...
            timer[0] = threading.Timer(t / 1000.0, delayed)
            timer[0].start()
...
#
# 解题思路:
# 使用threading.Timer实现防抖功能。每次调用防抖函数时，先取消之前的定时器，
# 再启动一个新的定时器。如果在t毫秒内没有新调用，定时器到期后执行原函数。
# 这与JavaScript的debounce语义一致：在等待期内重复调用会重置计时。
#
# 时间复杂度: O(1) 每次调用
# 空间复杂度: O(1)
#
# 关键点:
# - 使用threading.Timer模拟延迟执行
# - 每次新调用取消之前的定时器，重置等待时间
# - 参数t的单位是毫秒，需转换为秒用于threading.Timer
