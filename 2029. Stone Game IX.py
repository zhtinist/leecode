...
class Solution:
    def stoneGameIX(self, stones: List[int]) -> bool:
        # Count stones by remainder mod 3
        cnt = [0, 0, 0]
        for s in stones:
            cnt[s % 3] += 1

        # If cnt[0] is even, the game depends only on cnt[1] and cnt[2]
        # Alice can win if both cnt[1] >= 1 and cnt[2] >= 1
        if cnt[0] % 2 == 0:
            return cnt[1] >= 1 and cnt[2] >= 1
        else:
            # cnt[0] is odd: Alice can force a win if |cnt[1] - cnt[2]| > 2
...
# 解题思路:
# 将石子按模3的余数分类为0、1、2三类。
# 余数为0的石子可以用来"传递回合"，偶数个0相当于没起作用，奇数个0会反转局面。
# 当cnt[0]为偶数：Alice只需让序列以1和2交替开始即可获胜（需要两者都>=1）。
# 当cnt[0]为奇数：相当于有一次额外的回合传递，Alice需要|cnt[1]-cnt[2]|>2才能获胜。
#
# 时间复杂度: O(n)
# 空间复杂度: O(1)
#
