...
class Solution:
    ...
    def groupBy(self, arr: List, fn) -> dict:
...
#
# 解题思路:
# 遍历数组每个元素，调用fn获取分组键，使用defaultdict将元素追加到对应键的列表中。
# 保持元素原始顺序（因为我们按数组顺序遍历）。字典的键可以是任意字符串。
# 这与JavaScript的Array.groupBy(fn)语义一致。
#
# 时间复杂度: O(n)
# 空间复杂度: O(n)
#
# 关键点:
# - 使用defaultdict(list)简化追加操作
# - 顺序遍历保证每个组内元素顺序与原数组一致
# - fn返回字符串作为分组键
